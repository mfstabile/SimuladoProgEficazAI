...
def cadastrar_produto():
    ...
    conn = conectar_banco()
    cursor = conn.cursor()
    cursor.execute("INSERT INTO tbl_produtos (nome, descricao, preco) VALUES (?, ?, ?)", (dados["nome"], dados.get("descricao", ""), dados["preco"]))
    conn.commit()
    new_id = cursor.lastrowid
    cursor.close()
    conn.close()
    return jsonify({"produto_id": new_id}), 201
...
def adicionar_estoque():
    ...
    cursor.close()
    conn.close()
    return jsonify({"estoque_id": new_id}), 201
